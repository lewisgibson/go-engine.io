// Command gin runs an Engine.IO echo server mounted on a Gin router. Every message a
// session sends is echoed straight back to that session.
package main
...
func main() {
	...
	// gin.WrapH adapts the Engine.IO http.Handler into a gin.HandlerFunc. Gin's
	// catch-all token is "*name", so "/engine.io/*any" matches every sub-path the
