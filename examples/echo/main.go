// Command echo runs an Engine.IO echo server mounted on a labstack Echo v4 router. Every
// message a session sends is echoed straight back to that session.
package main
...
func main() {
	...
	// echo.WrapHandler adapts the Engine.IO http.Handler into an echo.HandlerFunc.
	// Echo's wildcard token is "*", so "/engine.io/*" matches every sub-path the
